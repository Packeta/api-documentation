...
This also applies for an error message [`IncorrectApiPasswordFault`](errors-reference#incorrectapipasswordfault).
:::
...
:::info
The method does not validate the `barcode` in any way. If you wish to create a barcode to use it on your labels, it is important that you use `packetId` prefixed by the letter `Z` e.g. `Z1234567890`.
:::
...
Returns label as string containing the PDF document (base64 encoded binary) if the external carrier allows it, ie. its `apiAllowed` [attribute](../home-delivery/carriers#carrier) is set to `true`.
...
Returns label as string containing the PNG image (base64 encoded binary) if the external carrier allows it, ie. its `apiAllowed` [attribute](../home-delivery/carriers#carrier) is set to `true`.
...
:::danger[Warning]
It is necessary to HTML unescape the string before printing. Otherwise the label may be rendered incorrectly.
:::
...
**Deprecated** - this method does not return full information for BDS.

Function prototype: `string packetCourierNumber(unsignedLong packetId)`

Returns a courier number if the external carrier allows it, ie. its `apiAllowed` [attribute](../home-delivery/carriers#carrier) is set to `true`.
...
- Some carriers support `203` dpi only.
...
Returns labels as string containing the PDF document (base64 encoded binary) if the external carrier allows it, ie. its `apiAllowed` [attribute](../pudo-delivery/packeta-pudos#carriers-list) is set to `true`.

The returned PDF page format is, by default, A4 with four 105x148 mm labels placed on each page. There is the optional argument `format` to specify the output PDF format. It accepts following values:
...
:::info
To avoid lost responses, set your request timeout to five seconds times the requested label count. E.g. 120 seconds for 24 labels. Typically, the labels will be generated much faster, but occasionally there may be delays.
:::
