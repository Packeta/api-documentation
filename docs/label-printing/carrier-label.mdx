...
Before you can print carrier labels, the **courier number** must first be obtained. This is essentially the packet's identification number within the carrier's system.

If you opt to print the external carrier's label directly, the following steps need to be undertaken:

1. Create a packet using the [createPacket()](../api-reference/api-methods-reference#createpacket) method.
2. Retrieve the packet's tracking code via the [packetCourierNumberV2()](../api-reference/api-methods-reference#packetcouriernumberv2) method.
3. Use one of the available methods to generate the packet's external carrier label:
    ...
    - [packetCourierLabelsPdf()](../api-reference/api-methods-reference#packetcourierlabelspdf)
    - [packetCourierLabelsZpl()](../api-reference/api-methods-reference#packetcourierlabelzpl)
...
Like Packeta labels, `PDF` and `PNG` labels are provided in base64 encoded form, and thus require decoding.

`ZPL` labels come with XML escaping as the ZPL format may include characters that are interpreted as special characters in XML. Because of this, ZPL labels need to be XML unescaped before printing.
