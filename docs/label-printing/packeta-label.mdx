...
As outlined in the section on [label types](../getting-started/label-types.mdx), Packeta labels are suitable for all packets, yet for packets designated to external carriers, we advise utilizing [direct labels](../getting-started/label-types.mdx#carrier-labels).

For printing **Packeta labels**, you can utilize the subsequent API methods:
...
`PDF` labels are delivered in a base64 encoded style and require decoding.
