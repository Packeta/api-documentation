...
Creating carrier PUDOs packets can be a little confusing.

To create a Carrier PUDOs packet you need two things:
1. `addressId` - `id` of the desired carrier to which the Carrier pick-up point belongs
2. `carrierPickupPoint` - code of the destination Carrier pick-up point

These can be obtained either from our [Widget](../pudo-delivery/widget) or from our data feeds. ([carriers list](../pudo-delivery/packeta-pudos#carriers-list)
and [carriers' PUDOs feed](../pudo-delivery/carriers-pudos))

To create a Carrier pick-up point packet, you have to pass the ID of the desired carrier into the `addressId` field, and the code of the Carrier pick-up point into the `carrierPickupPoint` field.
